...
# discard 'subzone' because it has too many types and 'planning_area' is more important
...
def main():
    X = data.loc[:, numerical_features+categorical_features+['price']]
    price_area = data.loc[:, ['price', 'planning_area']]
    ...
    # transform 'planning_area'. Calculate the mean price of each area and replace the area name with the mean price
    X['planning_area'] = X['planning_area'].str.lower()
    mean_price = X['price']/X['size_sqft']
    mean_price = pd.concat([X['planning_area'], mean_price], axis=1)
